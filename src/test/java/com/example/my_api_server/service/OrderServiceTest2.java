...
class OrderServiceTest2 {
    ...
    void createOrderConcurrencyTest(int concurrencyOrderUsers, int stock, long quantity)
        ...
        for (int i = 0; i < products.size(); i++) {
            ...
            Assertions.assertThat(p.getStock()).isEqualTo(expectedRemaining); //현재 재고랑 잔여 재고 같은지 확인
        }

        Assertions.assertThat(orderCount).isEqualTo(expectedSuccess); // 주문 수 검증
    }
}
